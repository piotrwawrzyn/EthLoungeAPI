...
const SendVerificationEmail = user => {
  const { email, permalink, verificationToken } = user;

  const markup = generateEmailMarkup(permalink, verificationToken);

  const mailOptions = {
    from: '"ethlounge" <noresponse@ethlounge.com>',
    to: email,
    subject: 'Verify your email to join ethlounge',
    html: markup
  };
...
const generateEmailMarkup = (permalink, verificationToken) => {
  ...
  <body>
    <table
      style="width:40%; margin: 0 auto; color: #1b1c1d"
      border="0"
      cellpadding="0"
      cellspacing="0"
    >
